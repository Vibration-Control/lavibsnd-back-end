        ...
        for j in range(number_of_modes):
            for k in range(number_of_modes):
                for l in range(number_of_neutralizers):
                    frequency_ratio = optimization_data.frequencies[i] / optimization_data.neutralizers[l].frequency
                    ...
                    if optimization_data.neutralizers[l].type in [0, 1, 2, 3]:
                        node = optimization_data.neutralizers[l].modal_position
                        modal_mass_array[j, k] += equivalent_mass * phi[j, node] * phi[k, node]
                        modal_damp_array[j, k] += equivalent_damp * phi[j, node] * phi[k, node]

                    if optimization_data.neutralizers[l].type == 4:
                        node = optimization_data.neutralizers[l].modal_position
                        tip = optimization_data.neutralizers[l].modal_position_tip
                        modal_mass_array[j, k] += equivalent_mass * (phi[j, tip] * phi[k, tip] - phi[j, node] * phi[k, tip] - phi[j, tip] * phi[k, node])
                        modal_damp_array[j, k] += equivalent_damp * (phi[j, tip] * phi[k, tip] - phi[j, node] * phi[k, tip] - phi[j, tip] * phi[k, node])
...
def equivalent_parameters(neutralizer, frequency_ratio, real_shear_module_ratio=0, loss_factor=0, dynamic_stifness=0.0, real_shear_module_at_system_frequency=0.0):
    ...
    if neutralizer.type == 2:
        denominator = (frequency_ratio**2.0 - 1.0) ** 2.0 + (2.0 * neutralizer.damp * frequency_ratio) ** 2.0
        equivalent_damp = neutralizer.mass * neutralizer.frequency * 2.0 * neutralizer.damp * frequency_ratio**4.0 / denominator
        equivalent_mass = -neutralizer.mass * (frequency_ratio**2.0 - (1.0 + (2.0 * neutralizer.damp * frequency_ratio) ** 2.0)) / denominator
